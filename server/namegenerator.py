...
def get_random_name():
    l = np.random.choice(left)
    r = np.random.choice(right)
    return l.capitalize() + "_" + r.capitalize()


def get_random_names(count):
    ls = np.random.choice(len(left), size=count, replace=False)
    rs = np.random.choice(len(right), size=count, replace=False)

    names = [left[ls[i]].capitalize() + "_" + right[rs[i]].capitalize() for i in range(count)]
    return names
